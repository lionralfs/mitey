...
const stop = () => {
  ...
  const del = (id) => {
    api
      .stopTracker(id)
      .then((result) => {
        const time = utils.minutesToHoursMinutes(
          result.tracker.stopped_time_entry.minutes
        );
        ...
        if (roundUp > 0) {
          ...
          api
            .editTimeEntry({id, minutes: minutes + minToAdd})
            .then((result) => {
              console.log(
                utils.formatSuccess(`Rounded to ${minutes + minToAdd} minutes.`)
              );
            })
            .catch((err) => {
              console.log(err);
