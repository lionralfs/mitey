                ...
                api.getCustomers({name})
                    .then(result => {
                        ...
                        if (result.length > 1) {
                            ...
                            console.log(chalk.green(`Found customer "${result[0].customer.name}".`));
        ...
        api.getProjects({id})
            .then(result => {
                ...
                if (result.length > 1) {
                    ...
                    console.log(chalk.green(`Found project "${result[0].project.name}".`));
                    prompt.get(confirmProps, (err, res) => {
                        if (res && res.confirm.match(/y[es]*/)) {
                ...
                api.getServices({name})
                    .then(result => {
                        ...
                        if (result.length > 1) {
                            ...
                            console.log(chalk.green(`Found service "${result[0].service.name}".`));
                            prompt.get(confirmProps, (err, res) => {
                                if (res && res.confirm.match(/y[es]*/)) {
                                    chooseNote({
                                        customer: project.customer_id,
            ...
            api.createTimeEntry({project, service, note})
                .then(result => {
                    api.startTracker({id: result.time_entry.id})
                        .then(result => {
                            if(result.tracker) {
                                console.log(chalk.green('Tracker is running!'));
                            } else {
