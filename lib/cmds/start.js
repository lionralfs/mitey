    ...
    prompt.get(
      {
        properties: {
          name: {
            description: utils.formatPrompt(
              'Search for a customer (or press Enter to see them all):'
            ),
          },
        },
      },
          ...
          api
            ...
            .then((result) => {
              ...
              if (result.length > 1) {
                ...
                prompt.get(
                  {
                    properties: {
                      number: {
                        message: utils.formatPrompt(
                          'Found more than one service. Enter a number.'
                        ),
                        type: 'integer',
                        required: true,
                        warning: `Must enter a number between 0 and ${result.length -
                          1}`,
                        minimum: 0,
                ...
                console.log(
                  utils.formatSuccess(
                    `Found customer "${result[0].customer.name}".`
                  )
                );
    ...
    api
      ...
      .then((result) => {
        ...
        if (result.length > 1) {
          ...
          prompt.get(
            {
              properties: {
                number: {
                  message: utils.formatPrompt(
                    'Found more than one project. Enter a number.'
                  ),
                  type: 'integer',
                  required: true,
                  warning: `Must enter a number between 0 and ${result.length -
          ...
          console.log(
            utils.formatSuccess(`Found project "${result[0].project.name}".`)
          );
          prompt.get(confirmProps, (err, res) => {
            if (res && res.confirm.match(confirmRegex)) {
              // Proceed
    ...
    prompt.get(
      {
        properties: {
          name: {
            description: utils.formatPrompt(
              'Search for a service (or press Enter to see them all):'
            ),
          },
        },
      },
          ...
          api
            ...
            .then((result) => {
              ...
              if (result.length > 1) {
                ...
                console.log(
                  utils.formatSuccess(
                    `Found service "${result[0].service.name}".`
                  )
                );
                prompt.get(confirmProps, (err, res) => {
                  if (res && res.confirm.match(confirmRegex)) {
    ...
    prompt.get(
      ...
      (err, result) => {
        ...
        api
          ...
          .then((result) => {
            api
              .startTracker({id: result.time_entry.id})
              .then((result) => {
                if (result.tracker) {
